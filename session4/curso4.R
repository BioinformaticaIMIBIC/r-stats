...
packages <- c("readxl","factoextra","FactoMineR","caret","car","biotools","MVN","forestmodel","see",
              ...
              "plot3Drgl","apaTables","gvlma","performance","MASS","faraway","reshape2","RColorBrewer")
...
library(parameters) #Activad este paqute
...
#Extraed una muestra aleatoria de 500 individuos a partir del archivo original y cread vuestro propio archivo de datos.
...
# paq:  jtools
library(jtools) #Poned este paquete
...
#o según el criterio de AKAIKE (AIC).
...
library(olsrr) #Ponedlo

model_parameters(linearMod, bootstrap = TRUE, iterations = 500)

# Otra forma de eliminar las variables no significativas
...
#Clase 4 Análisis de supervivencia-------------------------------------------------

#Importamos los datos
library(KMsurv)

data("kidtran")
kidtran <- as.data.frame(kidtran)
str(kidtran)
head(kidtran)

#Análisis exploratorio

with(kidtran, table(delta))  #Censura (0=Vivo, 1=Muerte)

with(kidtran, prop.table(table(delta)))


##Las covariables que representan grupos las convertiremos a tipo factor para
##un mejor uso y visualización.

kidtran$gender <- factor(kidtran$gender, labels = c("Hombre", "Mujer"))
kidtran$race <- factor(kidtran$race, labels = c("Blanco", "Negro"))
str(kidtran) #comprobamos

with(kidtran, table(gender, delta))  #Tabla por genero.

with(kidtran, table(race, delta))  # Tabla por raza.

with(kidtran, hist(age)) #Distribución de la edad.

##La variable edad se analizará creando una nueva variable edadg en donde se 
##dividirá en 3 grupos: Menores hasta 25, Mayores a 25 hasta 50 y mayores a 50.

kidtran$edadg <- cut(kidtran$age, c(0, 25, 50, 75))
with(kidtran, table(edadg, delta))


#Estimación No-Parametríca: Análisis exploratorio

##No paramétrica por género
library(survival)
library(survminer)
gen <- survfit(Surv(time, delta) ~ gender, kidtran, conf.type = "log-log")

ggsurvplot(title = "Supervivencia por Género",conf.int = T, legend.title = "Género", 
           legend.labs = c("Hombres", "Mujeres"), gen)


##No paramétrica por raza
raza <- survfit(Surv(time, delta) ~ race, kidtran, conf.type = "log-log")
ggsurvplot(title = "Supervivencia por Raza", conf.int = T, legend.title = "Raza", 
           legend.labs = c("Blanco", "Negro"), raza)

##No paramétrica por edad

edad <- survfit(Surv(time, delta) ~ edadg, kidtran, conf.type = "log-log")
ggsurvplot(title = "Supervivencia por Edad", conf.int = T, legend.title = "Edad", 
           legend.labs = c("0-25", "25-50", "50+"), edad)

##Ahora usando covariables
###Género y edad
cov1 <- survfit(Surv(time, delta) ~ gender + edadg, kidtran, conf.type = "log-log")
ggsurvplot(title = "Supervivencia entre Género por Edad", conf.int = T, 
             facet.by = "edadg", legend.title = "Género", 
           panel.labs = list(edadg = c("0-25","25-50", "75+")), short.panel.labs = T, cov1)


###Raza y edad
cov2 <- survfit(Surv(time, delta) ~ race + edadg, kidtran, conf.type = "log-log")
ggsurvplot(title = "Supervivencia entre Raza por Edad", conf.int = T, facet.by = "edadg", 
             legend.title = "Raza", 
           panel.labs = list(edadg = c("0-25", "25-50", "75+")), short.panel.labs = T, cov2)


###Género y raza
cov3 <- survfit(Surv(time, delta) ~ race + gender, kidtran, conf.type = "log-log") 
  ggsurvplot(title = "Supervivencia entre Género por Raza", conf.int = T, 
             facet.by = "gender", legend.title = "Raza", short.panel.labs = T, cov3)

###Raza, género y edad

cov4 <- survfit(Surv(time, delta) ~ race + gender + edadg, kidtran, conf.type = "log-log")
...
AIC.model[, order(AIC.model["AIC", ])] #El Modelo con menor AIC es el Gompertz


#Validamos con el Gráfico de Probabilidad Estabilizada  
...
#Gráficamente el modelo parece adecuado por lo cual optaremos por no desecharlo.
#Las estimaciones de los parámetros y sus intervalos de confianza son:
round(model$gompertz$res, 8)


#Modelo Semiparamétrico

##Comenzaremos con un modelo completo contemplando las variables age, race y 
#gender, e iremos eliminando las variables no significativas.
...
cox_model <- coxph(Surv(time, delta) ~ edadg, data = kidtran)


#Supuestos
# Test de Schoenfeld para verificar riesgos proporcionales
cox.zph(cox_model) #p > 0.05 , no hay evidencia de violación del supuesto de riesgos proporcionales.

ggcoxzph(cox.zph(cox_model)) #Los gráficos muestran valores constantes en el tiempo. Por lo que es correcto incluir tales covariables en el modelo de Cox
